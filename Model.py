...
class Model:
    def __init__(self):
        ...
        self.BR_HIST_SIZE = (
            500  # Fast breathing 20 breaths per minute, sampled once every breathing cycle, over 10 minutes this is 200 values
        )
    ...
    def update_hrv_spectrum(self):
        ...
        peak_indices = np.where(
            (hrv_psd_freqs_interp >= pacer_freq - 0.015) & (hrv_psd_freqs_interp <= pacer_freq + 0.015)
    ...
    def update_breathing_spectrum(self):
        ...
        peak_indices = np.where(
            (br_psd_freqs_interp >= pacer_freq - 0.015) & (br_psd_freqs_interp <= pacer_freq + 0.015)
        )  # 0.03 Hz around the peak is recommended by R. McCraty
